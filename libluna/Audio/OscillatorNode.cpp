...
void OscillatorNode::render(float *buffer, int frameCount) {
  ...
  float phaseIncrement =
      static_cast<float>(pi * 2.0f * mImpl->mFrequency / getFrameRate());
  ...
  for (int i = 0; i < frameCount; ++i) {
    ...
    mImpl->mPhase = fmodf(
        static_cast<float>(mImpl->mPhase + phaseIncrement),
...
void OscillatorNode::setType(Type type) { mImpl->mType = type; }

void OscillatorNode::setDuty(float duty) { mImpl->mDuty = duty; }
